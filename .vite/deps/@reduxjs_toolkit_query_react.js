...
async function defaultBackoff(attempt = 0, maxRetries = 5) {
  ...
  await new Promise(
    (resolve) => setTimeout((res) => resolve(res), timeout)
  );
}
function fail(e) {
...
function calculateProvidedBy(description, result, error, queryArg, meta, assertTagTypes) {
  if (isFunction(description)) {
    return description(
      ...
      queryArg,
      meta
    ).map(expandTagDescription).map(assertTagTypes);
  }
    ...
    try {
      ...
      if (forceQueryFn) {
        ...
        result = await endpointDefinition.queryFn(
          ...
          baseQueryApi,
          endpointDefinition.extraOptions,
          (arg2) => baseQuery(arg2, baseQueryApi, endpointDefinition.extraOptions)
        );
      }
...
In the case of an unhandled error, no tags will be "provided" or "invalidated".`,
  ...
  const prefetch = (endpointName, arg, options) => (dispatch, getState) => {
    ...
    const queryAction = (force2 = true) => api.endpoints[endpointName].initiate(
      arg,
      { forceRefetch: force2 }
    );
        ...
        updateQuerySubstateIfExists(
          ...
          (substate) => {
            ...
            if (merge) {
              ...
              substate.data = definitions[meta.arg.endpointName].structuralSharing ?? true ? copyWithStructuralSharing(
...
var defaultQuerySubState = produce(
  initialSubState,
  () => {
  }
);
var defaultMutationSubState = produce(
  initialSubState,
  () => {
  }
);
...
var defaultSerializeQueryArgs = ({
  ...
  if (typeof cached === "string") {
    ...
    const stringified = JSON.stringify(
      queryArgs,
      (key, value) => isPlainObject(value) ? Object.keys(value).sort().reduce((acc, key2) => {
        acc[key2] = value[key2];
        return acc;
      }, {}) : value
    );
...
function buildCreateApi(...modules) {
  return function baseCreateApi(options) {
    ...
    const initializedModules = modules.map(
      (m) => m.init(api, optionsWithDefaults, context)
    );
    ...
    context.batch(() => {
      for (const queryCacheKey of Object.keys(subscriptions)) {
        ...
        if (!subscriptionSubState || !querySubState)
          continue;
        const shouldRefetch = Object.values(subscriptionSubState).some(
          (sub) => sub[type] === true
...
var buildCacheLifecycleHandler = ({
  ...
  function handleNewKey(endpointName, originalArgs, queryCacheKey, mwApi, requestId) {
    ...
    const lifecycleApi = {
      ...
      requestId,
      extra,
      updateCachedData: endpointDefinition.type === "query" ? (updateRecipe) => mwApi.dispatch(
...
function buildHooks({
  ...
  moduleOptions: {
    batch,
    hooks: { useDispatch: useDispatch2, useSelector: useSelector2, useStore: useStore2 },
  ...
  function buildQueryHooks(name) {
    ...
    const useLazyQuerySubscription = ({
      ...
      usePossiblyImmediateEffect(() => {
        ...
        if (stableSubscriptionOptions !== lastSubscriptionOptions) {
          (_b = promiseRef.current) == null ? void 0 : _b.updateSubscriptionOptions(
            stableSubscriptionOptions
          );
        }
      }, [stableSubscriptionOptions]);
    ...
    const useQueryState = (arg, { skip = false, selectFromResult } = {}) => {
      ...
      const querySelector = (0, import_react.useMemo)(
        () => selectFromResult ? createSelector([selectDefaultResult], selectFromResult) : selectDefaultResult,
  ...
  function buildMutationHook(name) {
    return ({
      ...
      const mutationSelector = (0, import_react.useMemo)(
        () => createSelector(
          [select({ fixedCacheKey, requestId: promise == null ? void 0 : promise.requestId })],
          selectFromResult
        ),
...
function ApiProvider(props) {
  const [store] = import_react5.default.useState(
    () => configureStore({
      reducer: {
  ...
  (0, import_react4.useEffect)(
    () => props.setupListeners === false ? void 0 : setupListeners(store.dispatch, props.setupListeners),
    [props.setupListeners, store.dispatch]
  );
  return import_react5.default.createElement(Provider_default, { store, context: props.context }, props.children);
}
var createApi2 = buildCreateApi(
  coreModule(),
  reactHooksModule()
);
export {
